...
class RC_Mantis_API {
	...
	const PRIORITIES      = array( 'none', 'low', 'normal', 'high', 'urgent', 'immediate' );
	const SEVERITIES      = array( 'feature', 'trivial', 'text', 'tweak', 'minor', 'major', 'crash', 'block' );
	...
	public function create_issue( array $data ): array|WP_Error {
		...
		$body = array(
			...
			'project'     => array( 'id' => (int) $data['project_id'] ),
	...
	public function add_note( int $issue_id, string $text, string $view_state = 'public' ): array|WP_Error {
		...
		return $this->request(
			...
			array(
				'text'       => $text,
				'view_state' => array( 'name' => $view_state ),
			)
		);
	}
	...
	public function attach_file( int $issue_id, string $file_path, string $reporter_filename = '' ): array|WP_Error {
		...
		$response = wp_remote_post(
			$this->base_url . "/api/rest/issues/{$issue_id}/files",
			array(
				'timeout' => 30,
				'headers' => array(
