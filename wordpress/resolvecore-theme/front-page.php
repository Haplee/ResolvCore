...
<head>
	...
	============================================================ */
		...
		.rc-nav-dd-btn[aria-expanded="true"] {
			color: var(--rc-text);
		}
		...
		.rc-nav-dd-btn[aria-expanded="true"] .rc-nav-dd-caret {
		...
		.rc-nav-dd-menu a::after {
			display: none;
		}

		.rc-nav-dd-menu a:hover {
	...
	============================================================ */
		...
		.rc-hero-scroll {
			...
			color: var(--rc-muted);
			letter-spacing: .14em;
			animation: rcFadeUp .8s .6s ease both;
		...
		.rc-hero-scroll-mouse {
			width: 22px;
			height: 36px;
			border: 1.5px solid var(--rc-muted);
			border-radius: 12px;
			display: flex;
			justify-content: center;
			padding-top: 6px;
		...
		.rc-hero-scroll:hover .rc-hero-scroll-mouse {
			border-color: var(--rc-accent);
		}
	...
	============================================================ */
		...
		.rc-demo-btn.active .rc-demo-btn-dot {
			background: var(--rc-accent);
		}
		...
		.rc-demo-replay:hover {
			color: var(--rc-accent);
			border-color: rgba(0, 229, 160, .4);
		}

		.rc-demo-replay:disabled {
			opacity: .4;
			cursor: not-allowed;
		}
		...
		#rc-term-output::-webkit-scrollbar {
			width: 4px;
		}

		#rc-term-output::-webkit-scrollbar-thumb {
			background: var(--rc-border2);
			border-radius: 2px;
		}
		...
		.rc-demo-bar-fill.running::after {
			content: '';
			position: absolute;
			inset: 0;
			background: linear-gradient(90deg, transparent, rgba(255, 255, 255, .35), transparent);
			animation: rcBarShimmer 1.2s linear infinite;
		}
		...
		/* Estado "ejecutando": controles atenuados + badge en el terminal */
		.rc-demo-busy .rc-demo-plat:not(.active),
		.rc-demo-busy .rc-demo-btn:not(.active) {
			opacity: .45;
			cursor: not-allowed;
		}
		...
		.rc-term-status.running {
			color: var(--rc-accent);
		}

		.rc-term-status.running::before {
			content: '●';
			margin-right: 5px;
			animation: rcPulse 1s infinite;
		}

		/* Salida JSON simulada del diagnóstico */
		.rc-demo-aux {
			display: flex;
			gap: .5rem;
			flex-wrap: wrap;
		...
		.rc-demo-result {
			margin-top: 2.5rem;
			animation: rcFadeUp .4s ease;
		}

		.rc-demo-result[hidden] {
			display: none;
		}
	...
	============================================================ */
		...
		.rc-vuln-name a {
			color: var(--rc-accent2);
			text-decoration: none;
			border-bottom: 1px dotted rgba(0, 153, 255, .45);
		}

		.rc-vuln-name a:hover {
			color: var(--rc-accent);
		}
	...
	============================================================ */
		...
		.rc-download-card-ver {
			font-size: 11px;
			color: var(--rc-muted);
			font-family: var(--rc-mono);
		}

		.rc-download-card-size {
			font-size: 11px;
			color: var(--rc-muted);
		}
	...
	============================================================ */
		...
		.rc-flow-desc a {
			color: var(--rc-accent);
			text-decoration: none;
			border-bottom: 1px dotted rgba(0, 229, 160, .45);
		}

		.rc-flow-desc a:hover {
			color: var(--rc-text);
		}
	...
	============================================================ */
		...
		.rc-hero--split .rc-hero-content {
			max-width: 560px;
			text-align: left;
			margin: 0;
		}

		.rc-hero--split .rc-badge {
			margin-left: 0;
			margin-right: 0;
		}

		.rc-hero--split .rc-hero-sub {
			margin-left: 0;
			margin-right: 0;
		}

		.rc-hero--split .rc-hero-actions {
			justify-content: flex-start;
		}

		.rc-hero--split .rc-hero-stats {
			grid-column: 1 / -1;
			grid-template-columns: repeat(4, 1fr);
			gap: 1.5rem 1.25rem;
			justify-items: center;
		...
		.rc-hero-card {
			...
			box-shadow: var(--rc-shadow-lg);
			overflow: hidden;
			animation: rcFadeUp .9s .25s ease both;
		}
		...
		.rc-hcard-dot:nth-child(1) {
			background: #ff5f57;
		}

		.rc-hcard-dot:nth-child(2) {
			background: #febc2e;
		}

		.rc-hcard-dot:nth-child(3) {
			background: #28c840;
		}
		...
		.rc-hcard-pill.warn {
			color: #febc2e;
			border-color: rgba(254, 188, 46, .3);
		}
	   ...
	   para que el hover-lift no choque con un transform de keyframes. */
		...
		@media (prefers-reduced-motion: no-preference) {
			.rc-hcard-bar i {
				...
				animation: rcBarFill 1.1s .55s cubic-bezier(.4, 0, .2, 1) both;
			}
		}
		...
		/* Las metricas "en vivo" (JS) reajustan el ancho con suavidad. */
		.rc-hcard-bar i {
			transition: width .9s ease;
		}

		/* Scanline sutil que recorre la tarjeta (refuerza el indicador "en vivo"). */
		.rc-hero-card::before {
			content: '';
			position: absolute;
			left: 0;
			...
			top: -40%;
			height: 38%;
			background: linear-gradient(180deg, transparent, rgba(0, 229, 160, .05), transparent);
			animation: rcScan 7s linear infinite;
			pointer-events: none;
			z-index: 2;
		}
		...
		/* Tilt 3D gestionado por JS: transicion corta mientras se mueve el raton. */
		.rc-hero-card.tilting {
			transition: transform .12s ease-out, box-shadow .35s ease;
		}

		/* Subrayado tipo marcador bajo la palabra acentuada del H1. */
		.rc-hero h1 .accent {
			position: relative;
			display: inline-block;
		}

		.rc-hero h1 .accent::after {
			content: '';
			position: absolute;
			left: -2px;
			right: -2px;
			...
			transform: scaleX(0);
			transform-origin: left;
			animation: rcUnderline .7s .9s cubic-bezier(.4, 0, .2, 1) both;
		}

		@keyframes rcUnderline {
			to {
				transform: scaleX(1);
			}
		}
...
<body <?php body_class(); ?>>
	...
	<nav class="rc-nav" id="rc-nav" aria-label="Navegación principal">
		...
		<ul class="rc-nav-links">
			...
			<li class="rc-nav-dd">
				<button type="button" class="rc-nav-dd-btn" id="rc-dd-btn" aria-expanded="false" aria-haspopup="true"
					aria-controls="rc-dd-recursos">
	...
	<main id="main-content">
		...
		<section class="rc-hero rc-hero--split">
			...
			<div class="rc-hero-inner">
				<div class="rc-hero-content">
					...
					<p class="rc-hero-sub">Diagnóstico automatizado, proyección de vida útil del hardware y análisis de
				...
				<aside class="rc-hero-visual" aria-hidden="true">
					<div class="rc-hero-card">
						<div class="rc-hcard-head">
							<span class="rc-hcard-dot"></span><span class="rc-hcard-dot"></span><span
								class="rc-hcard-dot"></span>
				...
				<div class="rc-hero-stats">
					<div>
						...
						<div class="rc-stat-label">Respuesta inicial</div>
					</div>
					<div>
						<div class="rc-stat-num">3</div>
						<div class="rc-stat-label">Plataformas soportadas</div>
					</div>
					<div>
						<div class="rc-stat-num">7</div>
						<div class="rc-stat-label">Fases del proceso</div>
					</div>
					<div>
						<div class="rc-stat-num">GPL-3</div>
						<div class="rc-stat-label">Open Source</div>
					</div>
				</div>
			</div>
			<div class="rc-hero-scroll" aria-hidden="true">
				<div class="rc-hero-scroll-mouse">
					<div class="rc-hero-scroll-wheel"></div>
				</div>
				<span>SCROLL</span>
			</div>
		</section>
		...
		<div class="rc-platforms rc-reveal">
			<div class="rc-plat-inner">
				...
				<div class="rc-plat-items">
					<div class="rc-plat-item">
						<div class="rc-plat-icon pi-win">⊞</div> Windows 10 / 11
					</div>
					<div class="rc-plat-item">
						<div class="rc-plat-icon pi-lin">☰</div> Linux (Ubuntu, Debian, Arch)
					</div>
					<div class="rc-plat-item">
						<div class="rc-plat-icon pi-and">◈</div> Android 10+
					</div>
				</div>
			</div>
		</div>
		...
		<section class="rc-section" id="servicios" aria-label="Servicios">
			<div class="rc-reveal">
				...
				<p class="rc-section-desc">Una plataforma unificada para mantener tus sistemas en perfecto estado, con
					herramientas automatizadas de nivel profesional.</p>
			</div>
		...
		<section class="rc-section" id="como-funciona" aria-label="Cómo funciona">
			<div class="rc-reveal">
				...
				<p class="rc-section-desc">De la solicitud al cierre en 7 pasos. Proceso trazable, automatizado y
					documentado.</p>
			</div>
			<div class="rc-flow rc-reveal">
				...
				<div class="rc-flow-step">
					...
					<div class="rc-flow-desc">La solicitud se registra como incidencia con prioridad, categoría y número
						de seguimiento</div>
				</div>
				<div class="rc-flow-arrow">→</div>
				<div class="rc-flow-step">
					...
					<div class="rc-flow-desc">Acceso remoto seguro al equipo del cliente vía AnyDesk, previa
						autorización</div>
				</div>
				<div class="rc-flow-arrow">→</div>
				<div class="rc-flow-step">
					...
					<div class="rc-flow-desc">Scripts PowerShell/Bash recogen métricas del sistema y producen un
						diagnóstico estructurado en JSON</div>
				</div>
				<div class="rc-flow-arrow">→</div>
				<div class="rc-flow-step">
					...
					<div class="rc-flow-desc">Optimización, parches de seguridad y corrección de los fallos detectados
				...
				<div class="rc-flow-step">
					...
					<div class="rc-flow-desc">El JSON del diagnóstico pre-rellena una plantilla de informe .txt que el
						técnico completa y sube a la incidencia</div>
				</div>
				<div class="rc-flow-arrow">→</div>
				<div class="rc-flow-step">
					...
					<div class="rc-flow-desc">Al cerrar la incidencia se emite factura por intervención o cargo a la
		...
		<section class="rc-section" id="demo" aria-label="Demo interactiva">
			<div class="rc-reveal">
				...
				<p class="rc-section-desc">Elige plataforma y módulo para ver cómo actúa ResolveCore. Simulación con
					datos de ejemplo.</p>
			</div>
			<div class="rc-demo-layout rc-reveal">
				<div>
					<div class="rc-demo-platforms" role="tablist" aria-label="Plataforma de la demo">
						<button class="rc-demo-plat active" role="tab" aria-selected="true"
							onclick="selectPlatform('windows',this)"><span aria-hidden="true">⊞</span> Windows</button>
						<button class="rc-demo-plat" role="tab" aria-selected="false"
							onclick="selectPlatform('linux',this)"><span aria-hidden="true">☰</span> Linux</button>
						<button class="rc-demo-plat" role="tab" aria-selected="false"
							onclick="selectPlatform('android',this)"><span aria-hidden="true">◈</span> Android</button>
					</div>
					...
					<div class="rc-demo-progress">
						<div class="rc-demo-progress-label"><span id="rc-prog-label">Listo</span><span
								id="rc-prog-pct">—</span></div>
						<div class="rc-demo-bar" role="progressbar" aria-valuemin="0" aria-valuemax="100"
							aria-valuenow="0" aria-label="Progreso del análisis">
							<div class="rc-demo-bar-fill" id="rc-demo-bar"></div>
						</div>
					</div>
					<div class="rc-demo-aux">
						<button type="button" class="rc-demo-replay" id="rc-demo-replay" onclick="replayDemo()">↻
							Repetir análisis</button>
						<button type="button" class="rc-demo-replay" id="rc-demo-json" onclick="toggleDemoJson()"
							aria-pressed="false">{ } Ver JSON</button>
					</div>
				</div>
				<div class="rc-terminal">
					<div class="rc-term-header">
						...
						<span class="rc-term-status" id="rc-term-status" aria-hidden="true"></span>
					</div>
		...
		<section class="rc-section" id="precios" aria-label="Precios">
			...
			<div class="rc-pricing-grid rc-reveal">
				<div class="rc-pricing-card">
					...
					<div class="rc-pricing-price"><span class="price-currency">€</span><span
							class="price-num">0</span><span class="price-period">/ siempre</span></div>
					...
					<div style="margin-top:1.5rem"><a href="https://github.com/Haplee/ResolveCore" target="_blank"
							rel="noopener noreferrer" class="rc-btn-outline"
							style="width:100%;justify-content:center;font-size:11px;padding:10px">DESCARGAR EN GITHUB
							→</a></div>
				</div>
				<div class="rc-pricing-card featured">
					...
					<div class="rc-pricing-price"><span class="price-currency">€</span><span
							class="price-num">4.99</span><span class="price-period">/ mes</span></div>
					<div class="rc-pricing-divider"></div>
					...
					<div class="rc-pricing-feature"><span class="pf-none">─</span> Panel multiplataforma</div>
					<div style="margin-top:1.5rem"><a href="#contacto" class="rc-btn-primary"
							style="width:100%;justify-content:center;font-size:11px;padding:10px;display:flex">EMPEZAR
							PRO</a></div>
				</div>
				<div class="rc-pricing-card">
					<div class="rc-pricing-label">PLAN</div>
					...
					<div class="rc-pricing-price"><span class="price-currency">€</span><span
							class="price-num">14.99</span><span class="price-period">/ mes</span></div>
					<div class="rc-pricing-divider"></div>
					...
					<div class="rc-pricing-feature"><span class="pf-check">■</span> Panel multiplataforma <span
							class="rc-beta-tag">BETA</span></div>
					<div style="margin-top:1.5rem"><a href="#contacto" class="rc-btn-outline"
							style="width:100%;justify-content:center;font-size:11px;padding:10px;display:flex">CONTACTAR</a>
					</div>
				</div>
			</div>
		</section>
		...
		<section class="rc-section" id="contacto" aria-label="Contacto">
			<div class="rc-reveal">
				...
				<p class="rc-section-desc">Elige el canal que prefieras. Para abrir un ticket de soporte crea tu cuenta
					de cliente — respuesta inicial en menos de 2 horas.</p>
			</div>
			...
			<div class="rc-quick-channels rc-reveal">
				...
				<a class="rc-channel" href="https://github.com/Haplee" target="_blank" rel="noopener noreferrer"
				...
				<a class="rc-channel" href="https://x.com/FranVidalMateo" target="_blank" rel="noopener noreferrer"
					aria-label="Perfil en Twitter / X">
					<div class="rc-channel-icon" aria-hidden="true">
						<svg viewBox="0 0 24 24" width="18" height="18" fill="currentColor">
							<path
								d="M18.2 2H21l-6.5 7.4L22 22h-6l-4.7-6.1L5.9 22H3l7-8L2.2 2h6.1l4.2 5.6L18.2 2Zm-2.1 18h1.7L7.9 3.8H6.1L16.1 20Z" />
						</svg>
					</div>
					<div class="rc-channel-body">
	...
	<script>
		...
		const demoPlatforms = {
			windows: {
				label: 'Windows', prompt: 'PS C:\\>',
				cmd: {
					diagnostico: '.\\diagnostico.ps1 -Ticket 42',
			...
			linux: {
				label: 'Linux', prompt: 'rc@linux:~$',
				cmd: {
					diagnostico: 'bash diagnostico.sh --ticket 42',
			...
			android: {
				label: 'Android', prompt: 'rc@android:/ $',
				cmd: {
					diagnostico: 'bash diagnostico.sh --ticket 42',
		...
		const demoModules = {
			...
			optimizacion: {
				...
				lines: [
					...
					['info', {
						windows: '→ 23 procesos de inicio innecesarios',
					...
					['ok', {
						windows: '✓ 12 procesos de inicio desactivados',
		...
		/* Estado visual "ejecutando": atenúa controles, badge del terminal y shimmer. */
		function setDemoBusy(busy) {
			const layout = document.querySelector('.rc-demo-layout');
			if (layout) layout.classList.toggle('rc-demo-busy', busy);
			const bar = document.getElementById('rc-demo-bar');
			if (bar) bar.classList.toggle('running', busy);
			const status = document.getElementById('rc-term-status');
			if (status) {
				status.textContent = busy ? 'ejecutando' : 'listo';
				status.classList.toggle('running', busy);
			}
			const jsonBtn = document.getElementById('rc-demo-json');
			if (jsonBtn) jsonBtn.disabled = busy;
		}
		...
		function animateCount(el, target, dur, from) {
			const start = performance.now();
			const base = from || 0;
			(function tick(now) {
				const p = Math.min(1, (now - start) / dur);
				el.textContent = Math.round(base + p * (target - base));
		...
		function showDemoResult(mod, instant) {
			...
			demoState.score = mod.score; /* punto de partida: reparar CVEs lo sube en vivo */
	...
	<div class="rc-ticket-modal" id="rc-ticket-modal" role="dialog" aria-modal="true"
		aria-labelledby="rc-ticket-modal-title" aria-hidden="true">
	...
	<style>
		...
		.rc-ticket-spin {
			width: 32px;
			height: 32px;
			border: 2px solid var(--rc-border2);
		...
		.rc-track-step:last-child::before {
			display: none;
		}

		.rc-track-step.done::before {
			background: var(--rc-accent);
		}
		...
		.rc-track-step.done .rc-track-dot::after {
			content: '✓';
		}
		...
		@keyframes rcDotPulse {
			50% {
				box-shadow: 0 0 0 8px rgba(0, 229, 160, .05);
			}
		}
		...
		.rc-track-step.done .rc-track-label {
			color: var(--rc-text);
		}

		.rc-track-step.active .rc-track-label {
			color: var(--rc-accent);
		}
		...
		.rc-chip {
			...
			padding: 3px 8px;
			border-radius: 3px;
			background: rgba(122, 127, 142, .13);
			color: var(--rc-muted);
			border: 1px solid var(--rc-border);
		}

		.rc-chip.priority-high,
		.rc-chip.priority-urgent,
		.rc-chip.priority-immediate {
			background: rgba(255, 107, 53, .1);
			color: var(--rc-warn);
			border-color: rgba(255, 107, 53, .3);
		}

		.rc-chip.priority-normal {
			background: rgba(0, 229, 160, .08);
			color: var(--rc-accent);
	...
	<style>
		...
		.rc-fab-contact::after {
			...
			animation: rcFabPulse 2.4s ease-out infinite;
			pointer-events: none;
		}

		@keyframes rcFabPulse {
			0% {
				box-shadow: 0 0 0 0 rgba(0, 229, 160, .45);
			}
		...
		body.rc-at-contact .rc-fab-contact {
			opacity: 0;
			pointer-events: none;
