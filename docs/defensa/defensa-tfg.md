...
| 2026-05-30 | **Registro de clientes provisionado + mejora visual home** (tema `resolvecore-theme`). (1) **Bug en producción**: el enlace «Acceso clientes → /registro/» daba 404 porque la plantilla `page-registro.php` existía pero ninguna página WP la usaba (no había auto-provisión). Nueva `rc_provision_pages()` en `functions.php` (idempotente, guard `rc_pages_provision_ver` + hooks `after_switch_theme`/`init`): crea las páginas que el nav da por hechas (`/registro/` con `[rc_registro_cliente]`, `/dashboard/` con `[rc_cliente_dashboard]`, y docs/changelog/fleet-status/tecnicos/contacto/legales con su plantilla); si la página ya existe solo corrige `_wp_page_template`. (2) **Mejora visual de la portada** (color verde de marca conservado): hero rehecho a **dos columnas** (copy a la izquierda + tarjeta-mockup de diagnóstico a la derecha: anillo de salud 87, barras CPU/Memoria/Disco, píldoras de estado) con grid `1.05fr/.95fr` que apila en ≤880px; botón secundario del hero ahora «CREAR CUENTA» → `/registro/`. Sección **demo interactiva eliminada** (home más corta y directa); `runDemo()` gana guard `if(!output||!bar) return` para no romper el resto del JS al faltar sus nodos. Tarjetas de **servicios** refinadas: icono protagonista (52px, fondo/borde de acento, lift en hover), más padding, barra de acento superior animada. PHP `-l` limpio en `front-page.php` y `functions.php`. |
