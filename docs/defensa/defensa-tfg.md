...
3. **Informe técnico en texto plano (`.txt`)** entregable al cliente, con proyección de vida útil del hardware y recomendaciones.
...
4. Resolver y entregar informe técnico en texto plano (`.txt`).
...
- **Flujo de datos de la home redescrito.** Los siete pasos de «El flujo de servicio» (`front-page.php`) describían acciones sueltas; ahora narran cómo viaja el dato (solicitud → incidencia con nº de seguimiento → diagnóstico JSON → el JSON pre-rellena la plantilla de informe `.txt` que el técnico completa → factura al cerrar) y se eliminó la marca «MantisBT» de la cara pública, dejándola como «incidencia».
- *Pendiente (follow-up):* el bloque de adjuntos del dashboard aún enlaza a `file_download.php` del servidor de tickets; requiere un proxy de descarga en WordPress para que el cliente baje el informe `.txt` sin cuenta en MantisBT.
...
- Diagnóstico completo + informe técnico (`.txt`): 29 €.
- Resolución de incidencia: 49 €/hora.
- Facturación al cerrar ticket en MantisBT.
...
- **Lenguaje:** Bash sobre ADB (o Termux). **Función:** TUI de diagnóstico,
  optimización y CVE del dispositivo Android. Al arrancar pide **una vez** el
  ticket MantisBT de la sesión (igual que los menús Windows/Linux) y lo pasa
  con `--ticket` a diagnóstico y optimización, de modo que las salidas caen en
  `diagnosticos/tickets/<NNNNN>/android/`.
- **Requisitos:** `adb` instalado y dispositivo autorizado (Depuración USB).
- **Ejecución:** `bash scripts/android/ResolveCore.sh` (menú) ·
  `bash scripts/android/ResolveCore.sh --ticket 42 estandar` (pass-through)
...
| Fecha | Cambio |
|-------|--------|
| 2026-06-12 | **Score de salud del servidor visible en los 3 agentes de diagnóstico.** El score 0-100 lo calcula SIEMPRE el servidor (`rc_fleet_score()` en `wordpress/plugins/rc-fleet/rc-fleet.php`) cuando el agente publica el JSON en `POST /wp-json/rc/v1/fleet`, y viene en la respuesta (`{ok, action, id, host_id, score}`); el script del cliente NO lo calcula. Hasta hoy solo `diagnostico.ps1` lo extraía de la respuesta; Linux y Android volcaban el JSON crudo de la respuesta sin destacarlo. Ahora los 3 scripts (`windows/diagnostico.ps1`, `linux/diagnostico.sh`, `android/diagnostico.sh`) muestran tras la subida una línea uniforme «Puntuación de salud (servidor): N/100» — en Bash se extrae con `sed -n` (sin dependencia de `jq`); si la respuesta no trae `score` (p. ej. acta de optimización) no se imprime nada. Sin cambios en el JSON de diagnóstico generado (el schema no se toca) ni en la lógica de subida. Validación: `bash -n` limpio ×2, parser PowerShell limpio, BOM UTF-8 intacto en el `.ps1`, extracción probada contra respuesta real y contra respuesta sin score. Si el tribunal pregunta dónde se calcula el score: **en el servidor, plugin rc-fleet** (`guion-oral-defensa.md` líneas 62 y 226 ya lo recogen). |
| 2026-06-10 | **Fix Android: el launcher nunca preguntaba el ticket** (`scripts/android/ResolveCore.sh`, v2.0→**2.1**). Los subscripts `android/diagnostico.sh` y `android/optimizacion.sh` ya soportaban `--ticket N`, pero el menú del técnico ni lo pedía ni lo reenviaba, así que **todas** las salidas Android caían en `diagnosticos/tickets/sin-ticket/android/` y la estructura por ticket no funcionaba en esta plataforma. Portado el patrón de sesión de Linux/Windows: global `TICKET_SESION` + función `read_ticket_sesion()` que pregunta el ticket **una vez al arrancar el menú** (valida `^[0-9]+$`; ENTER = sin ticket), y paso de `--ticket` en `run_diagnostico()` y `run_optimizacion()`. Corregido además un bug del modo pass-through: `--ticket` no estaba en el `case` de argumentos y caía en `*)`, interpretándose como **serial ADB**; ahora se parsea (`TICKET_CLI`), se reenvía a ambos módulos y, si se llega al menú con `--ticket` ya dado, no se vuelve a preguntar. Ayuda (`show_usage`/`show_help`) actualizada. Validación: `bash -n` limpio, `--help` muestra la flag, pass-through con serial falso construye bien el comando (falla limpio en el check de dispositivo). **Pendiente de validar por el autor con dispositivo real**: menú con ticket 42 → JSON en `diagnosticos/tickets/00042/android/`. Anotado (sin tocar): `linux/ResolveCore.sh:827` tampoco pasa `--ticket` a su `optimizacion.sh` (Windows sí lo hace). |
| 2026-06-10 | **Reparación de la demo interactiva de la home** (`front-page.php`). La demo llegó rota a `main`: el commit que la mejoró (entrada anterior de hoy) llamaba a `resetDemoJson()`/`toggleDemoJson()` **sin definirlas**, y el `ReferenceError` en el render inicial dejaba `demoState.running = true` para siempre → label congelado en «Ejecutando diagnóstico…», porcentaje en «—» y todos los botones (plataforma, módulo, repetir, JSON) inertes. Implementadas las 3 funciones que faltaban: `buildDemoJson()` (objeto `_meta` + `salud`/`resumen`/`eventos`/`vulnerabilidades` construido desde `demoModules`/`demoState`, espejo informal del schema de diagnóstico), `toggleDemoJson()` (alterna terminal ↔ JSON con línea `cat diagnosticos/tickets/00042/<plataforma>/diagnostico.json`, snapshot del terminal para restaurar, `aria-pressed`, deshabilitado mientras corre la simulación; sin el coloreado por claves que prometía la entrada anterior — render monoespaciado plano) y `resetDemoJson()` (restaura vista y botón; `runDemo()` la invoca al lanzar cada simulación). Validación: `php -l` limpio, bloques `<script>` extraídos y verificados con `node --check`, los 5 handlers `onclick` de la página tienen definición. Pendiente de validar por el autor en navegador (LocalWP): animación, toggle JSON y reparación de CVEs en vivo. |
| 2026-06-10 | **Mejora de la demo interactiva y del hero de la home** (`front-page.php`). Demo: estado «ejecutando» visible (badge pulsante en la cabecera del terminal, shimmer en la barra de progreso, controles atenuados mientras corre la simulación); reparar un CVE ahora tiene impacto en vivo (sube el gauge de salud y la stat «Salud /100» según severidad — crit +9 / high +6 / med +4 — y añade línea `✓ CVE-XXXX aplicado` al terminal); nuevo botón **«{ } Ver JSON»** que muestra en el terminal un `cat diagnosticos/tickets/00042/<plataforma>/diagnostico.json` con el JSON simulado del escenario (espejo del schema real, con `_meta.plataforma`), coloreado por claves/strings/números; navegación con flechas ←/→ entre tabs de plataforma (a11y). Hero: tilt 3D sutil de la tarjeta de diagnóstico al mover el ratón (solo puntero fino, respeta `prefers-reduced-motion`), métricas CPU/RAM/Disco que oscilan «en vivo» cada 2,4 s coherentes con el punto «en vivo» del título, scanline que recorre la tarjeta, y subrayado tipo marcador animado bajo la palabra «problemas» del H1. Sin cambios de backend ni de flujo: todo es presentación/simulación en la home. |
| 2026-06-07 | **Estructura de salidas por plataforma + fix carpeta basura `-Ticket`.** Las salidas pasan de `diagnosticos/tickets/<NNNNN>/` a **`diagnosticos/tickets/<NNNNN>/<plataforma>/`** (`windows`/`linux`/`android`), tanto con ticket como en `sin-ticket/`. Aplicado en los 6 scripts de diagnóstico/optimización (`diagnostico.{ps1,sh}`, `optimizacion.{ps1,sh}` ×3 plataformas), en `generar_informe.py` (nuevo flag `--plataforma`; deduce de `_meta.plataforma` del `--json`; fallback `sin-plataforma`) y en los launchers `ResolveCore.{ps1,sh}` (lectura del JSON más reciente y paso de `--plataforma` al informe). **Bug carpeta `-Ticket`**: se creaba `scripts/windows/-Ticket/` cuando `-OutputDir` recibía la cadena literal `"-Ticket"` (binding posicional: `OutputDir` declarado antes que `Ticket`, ambos `[string]`); añadido **blindaje** en `diagnostico.ps1`/`optimizacion.ps1` que ignora un `-OutputDir` que empiece por `-`, y borrada la carpeta basura. Docs de rutas actualizadas (CLAUDE.md, docstrings de los scripts). **Pendiente de validar por el autor en Windows**: `diagnostico.ps1 -Ticket N` y `optimizacion.ps1 -Confirm -Ticket N` generan en `<NNNNN>/windows/`. |
| 2026-06-07 | **Auditoría de scripts (encoding + `-Silent`) — 0 errores de sintaxis.** Barrido estático (`bash -n`, `py_compile`, parser PowerShell) → 0 errores; hallazgos de encoding/documentación, no de lógica. **(1) BOM UTF-8 en `optimizacion.ps1`**: único `.ps1` con no-ASCII sin BOM (acentos, em-dashes `──`), que en PS 5.1 se lee como ANSI y rompe el render — corrige la afirmación del 2026-06-03 que daba los 7 `.ps1` por verificados; reescrito anteponiendo `EF BB BF` sin tocar contenido (CRLF preservado). **(2) `-Silent` en la subida de `diagnostico.ps1`**: las 3 líneas informativas del bloque de subida (`Write-Host` "Subiendo…", "Subida OK…", "(omitida…)") protegidas con `if (-not $Silent)`; los `Write-Warning` de fallo se conservan visibles. **(3) `.claude/CLAUDE.md`**: alineadas las referencias a `generar_factura.py` (archivado en `_archivo/common/` desde el 2026-06-04) — comando movido al bloque "ARCHIVADO", entrada del árbol `scripts/common/` retirada y §4 a "la facturación se gestiona desde MantisBT". **Pendiente de validar por el autor en Windows** (sin pwsh en el entorno): parser PS y render de acentos de `optimizacion.ps1`, y `diagnostico.ps1 -Silent` sin la línea "(Subida automática omitida…)". |
...
| 2026-06-15 | **Sincronización entregable cliente PDF → `.txt` (sitio web + este documento)**. Un smoke test de las rutas públicas de `resolvecore.website` destapó copy desactualizado que aún vendía el informe al cliente como **PDF**, contradiciendo la decisión del 2026-06-02 (informe/factura en `.txt` rellenado a mano, sin PDF). **Tema WP** (`resolvecore-theme`): `page-changelog.php` (entrada v1.2.0 "generador PDF wkhtmltopdf" → plantilla `.txt` `generar_informe.py` que el técnico sube a mano; flujo 7 fases "→ PDF →" → "→ informe →"), `page-contacto.php` ("informe PDF al cerrar" → "informe técnico"), `front-page.php` (feature "Informe exportable en PDF/JSON" → "Informe técnico en .txt + salida JSON"), `page-tecnicos.php` (widget subir informe `PDF/HTML` → `.txt/PDF/HTML`, `accept=".txt,.pdf,.html"`). **Backend** (`functions.php`): `rc_tech_upload_informe()` rechazaba el `.txt` (allowlist MIME solo `application/pdf`+`text/html`) — **bug funcional**, bloqueaba el deliverable real → añadido `text/plain`. **Este documento**: pilares §sinopsis, objetivos del flujo, narración del flujo de datos de la home, follow-up del proxy de descarga y modelo de precios/facturación B2C, todos de "PDF" → "`.txt`". **Conservado como legacy intencional** (no se toca): sección "Legacy generador HTML→PDF", `reports/generate-report.php`, infra wkhtmltopdf del VPS y la maquetación del propio TFG a PDF/Word para el tribunal. PHP `-l` limpio en los 5 ficheros del tema. |
