...
### Confirmación al cliente por correo
Al crear el ticket, el handler `resolvecore_handle_contact()` envía un **segundo correo al cliente** (`resolvecore_send_client_confirmation()`) además del aviso al técnico. Es un correo HTML con la estética del sitio (cabecera oscura, acento verde) que incluye: número de incidencia `#ID`, categoría, el mensaje enviado, las 4 fases de seguimiento y un botón **«Ver estado en tiempo real»**. El botón enlaza a `home_url('/?rc_ticket=ID&rc_t=TOKEN#contacto')`; un bloque JS en `front-page.php` lee los parámetros `rc_ticket` y `rc_t` y abre automáticamente el modal de tracking. Canal informativo no bloqueante: si `wp_mail()` falla solo se registra en el log, sin alterar la respuesta AJAX. `Reply-To` apunta al correo del técnico para que el cliente pueda ampliar la incidencia respondiendo al mensaje.
...
| 2026-05-21 | **Correo saliente en producción — relay Ionos + mejoras de entregabilidad**: el despliegue real destapó que Ionos bloquea el puerto 25 saliente del VPS (`Connection timed out`, correo en cola). Se configuró Postfix para relayar el correo autenticado por el smarthost `smtp.ionos.es:587` (buzón `tecnicos@`); `setup-mail-dkim.sh` ganó el flag `--relayhost` (relay + SASL, contraseña pedida de forma interactiva) y fija `myhostname = mail.<dominio>` + `mydestination` sin el dominio raíz, lo que corrige el rebote `unknown user` de los buzones del dominio. `resolvecore_send_client_confirmation()` pasó a `multipart/alternative` — añade una parte de texto plano junto al HTML — y emite la cabecera `List-Unsubscribe`. Doc `docs/tecnica/correo-dkim.md` ampliada con la sección "1b. Relay saliente". Verificado con mail-tester sobre el ticket #8: SPF/DKIM/DMARC autenticados, sin blacklist, enlace de seguimiento con token HMAC respondiendo 200. |
